...
# Lab7
Задачу реализовать двумя способами. Для первого способа использовать только свои функции для поиска max, min,
sum и т.д. без использования стандартных функций. Во втором способе использовать стандартные функции для работы с массивами. Сравнить
полученные программы по структурам и скорости выполнения.  
##### В массиве размером n (1<n<30) найти сумму всех отрицательных элементов. Вывести на экран массив и сумму.  

# Lab8-9
### task1
Задачу реализовать двумя способами. Для первого способа использовать только свои функции для перестановки, замены элементов
массива и т.д. без использования стандартных функций. Во втором способе использовать стандартные функции для работы с массивами. Сравнить полученные программы по структурам и скорости выполнения.  
#####  В матрице размером n на m (1<n,m<10) (ввод n и m с клавиатуры) найти сумму всех отрицательных элементов. Вывести на экран матрицу и сумму.  
### task2
Для сортировки использовать только собственные функции! (встроенные в php функции сортировки нельзя)
+ В матрице размером n на m (1<n,m<10) (ввод n и m с клавиатуры)
...
# Lab10-11
### task1
Дана строка найти и заменить в строке все сочетания (вводятся с формы) на сочетания (вводятся с формы). Вывести исходную строку и
получившуюся. Если искомое сочетание не встречается, то ответить, что заменять нечего.
### task2
Создать форму-анкету для заполнения личных данных начинающего писателя. Предусмотреть сбор следующей
информации о писателе: фамилия, имя, псевдоним, дата рождения, жанр его работ, отрывок из собственного произведения. Предусмотреть
корректность ввода данных. После оценки его произведения по критериям (у каждого варианта свои критерии), сформировать письмо с ответом о сотрудничестве или не сотрудничестве с данным писателем. Форму с письмом ответом разместить в отдельном файле типа html. Шаблон ответа
сформировать заранее. 
##### Разделить текст на две равные части. В первой и второй части посчитать количество слов. Если разность количества слов четная, то формировать письмо о сотрудничестве, иначе о не сотрудничестве.  

# Lab12-13
Написать PHP-скрипт, который должен читать список файлов из определенной папки, отбирать из них текстовые файлы и их
содержание выводить на страницу. Предусмотреть возможность:  
+ добавление файлов в папку;
+ удаление файлов из папки;
+ обработку выбранного файла по определенным правилам. 
Результат выводить в отдельном html документе. Предусмотреть достойное оформление главной web-страницы с применением CSS стилей!
##### Удалить из файла все вещественные числа из промежутка от 0.001 до 1.001.

# Lab14-15
Создать новостной сайт “Новости твоего города”. Сайт должен содержать перечень актуальных событий города,
например, города Гродно.  
На главной странице должны быть отображены 10 последних
актуальных статей (анонс).  
Пример оформления каждой новости на главной странице: название статьи, фото, относящееся к статье и краткое
содержание.  
Название статьи оформить в виде ссылки, переход по которой позволит прочитать всю статью и просмотреть все фотографии, относящиеся
к данной статье.  
Предусмотреть возможность авторизации и регистрации пользователей на сайте.
+ Администраторы сайта должны иметь возможность добавить, изменить или удалить любую статью на сайте.
+ Пользователи (зарегистрированные посетители) сайта имеют возможность читать краткое и полное содержание любой статьи.
+ Гости (не зарегистрированные посетители) имеют возможность просмотра только краткого содержания актуальных статей.  
Все данные о пользователях и о статьях должны храниться в базе.
